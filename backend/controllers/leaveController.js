...
const applyLeave = async (req, res) => {
  try {
    ...
    // Real-time update - Target requester and Admin
    req.io
      .to(`user:${req.user._id.toString()}`)
      .to('role:Admin')
      .emit('leave_updated', populatedLeave);
...
const updateLeaveStatus = async (req, res) => {
  try {
    ...
    // Real-time update - Target requester and Admin
    const requesterUserId = populatedLeave?.user?._id
      ? String(populatedLeave.user._id)
      : String(leave.user);
