...
const getQrPublicBaseUrl = () => {
    const configuredUrl = process.env.QR_PUBLIC_BASE_URL || process.env.FRONTEND_URL || 'http://localhost:5173';
...
const generatePatientQrShareLink = async (req, res) => {
    ...
    const patient = await User.findById(patientId).select('firstName lastName role');
...
const resolvePatientQrShareDetails = async (req, res) => {
    ...
    const patient = await User.findById(patientId)
        .select('firstName lastName email phone gender dateOfBirth profileImage patientId createdAt role')
        .lean();
    ...
    if (!consumedToken) {
        const existingToken = await QrShareToken.findOne({ patient: patientId, tokenHash }).lean();
        ...
        res.status(409);
        throw new Error('Unable to validate QR link. Please generate a new QR code.');
    }

    const [medicalHistory, prescriptions, labReports, appointments] = await Promise.all([
