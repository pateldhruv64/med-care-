...
const Reports = () => {
  ...
  const fetchAll = useCallback(
    async ({ showLoader = false, showError = true } = {}) => {
      if (showLoader) {
        setLoading(true);
      }
  ...
  useEffect(() => {
    fetchAll({ showLoader: true, showError: true });
  }, [fetchAll]);

  const queueReportsRefresh = useCallback(() => {
    if (refreshTimerRef.current) {
      clearTimeout(refreshTimerRef.current);
    }

    refreshTimerRef.current = setTimeout(() => {
      fetchAll({ showLoader: false, showError: false });
  ...
  useEffect(() => {
    ...
    socket.on('invoice_updated', queueReportsRefresh);
    socket.on('appointment_created', queueReportsRefresh);
    socket.on('appointment_status_updated', queueReportsRefresh);

    return () => {
      socket.off('invoice_updated', queueReportsRefresh);
      socket.off('appointment_created', queueReportsRefresh);
      socket.off('appointment_status_updated', queueReportsRefresh);
  ...
  const pendingAppts = appointments.filter(isPendingAppointment).length;
  ...
  const statusCounts = {
    Pending: appointments.filter(isPendingAppointment).length,
