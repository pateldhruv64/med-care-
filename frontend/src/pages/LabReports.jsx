...
const LabReports = () => {
  ...
  const filteredReports = reports.filter((r) => {
    ...
    const matchSearch =
      ...
      r.testName?.toLowerCase().includes(normalizedQuery) ||
      r.testCategory?.toLowerCase().includes(normalizedQuery) ||
      r.patient?.firstName?.toLowerCase().includes(normalizedQuery) ||
      r.patient?.lastName?.toLowerCase().includes(normalizedQuery) ||
      r.doctor?.firstName?.toLowerCase().includes(normalizedQuery) ||
      r.doctor?.lastName?.toLowerCase().includes(normalizedQuery);
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        {(user?.role === 'Doctor' || user?.role === 'Admin') && (
          <button
            ...
            className="w-full sm:w-auto bg-purple-500 hover:bg-purple-600 text-white px-5 py-2.5 rounded-lg flex items-center justify-center gap-2 font-bold transition-colors shadow-lg shadow-purple-500/20"
          >
            <Plus size={20} />
            Order Test
          </button>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          {
            label: 'Ordered',
            value: reports.filter((r) => r.status === 'Ordered').length,
            color: 'bg-orange-500',
            icon: Clock,
          },
          {
      ...
      <div className="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col sm:flex-row gap-3 sm:gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder={
              isPatient
                ? 'Search by test, category, or doctor...'
                : 'Search by test, patient, or doctor...'
            }
            value={searchTerm}
        ...
        <div className="flex gap-2 overflow-x-auto sm:overflow-visible scrollbar-soft pb-1">
        ...
        <div className="divide-y divide-slate-100">
          {filteredReports.length > 0 ? (
            filteredReports.map((report) => (
              <div key={report._id} className="px-4 py-4 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span
                    className={`px-2.5 py-1 rounded-full text-[11px] font-bold whitespace-nowrap ${statusColors[report.status]}`}
                  >
                    {report.status}
                  </span>
                </div>
                ...
                <div className="flex items-center flex-wrap gap-2">
                  <span
                    className={`px-2.5 py-1 rounded-full text-[11px] font-medium ${categoryColors[report.testCategory] || 'bg-gray-50 text-gray-600'}`}
                  >
                    {report.testCategory}
                  </span>
                ...
                {!isPatient && (
                  <p className="text-sm text-slate-600">
                    Patient: {report.patient?.firstName}{' '}
                ...
                <button
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-slate-100 text-slate-700 text-sm font-medium hover:bg-slate-200 transition-colors"
                >
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredReports.map((report) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-medium ${categoryColors[report.testCategory] || 'bg-gray-50 text-gray-600'}`}
                    >
                      {report.testCategory}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-bold ${statusColors[report.status]}`}
                    >
                      {report.status}
                    </span>
                  </td>
        ...
        {showCreateModal && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto"
            >
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium text-slate-800">
                      {showResultModal.patient?.firstName}{' '}
                      {showResultModal.patient?.lastName}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-500">Doctor</p>
                    <p className="font-medium text-slate-800">
                      Dr. {showResultModal.doctor?.firstName}{' '}
                      {showResultModal.doctor?.lastName}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-500">Category</p>
                    <p className="font-medium text-slate-800">
                      {showResultModal.testCategory}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-500">Ordered On</p>
                    <p className="font-medium text-slate-800">
                      {new Date(showResultModal.createdAt).toLocaleDateString()}
                    </p>
                  </div>
                </div>
                ...
                {user?.role === 'Doctor' || user?.role === 'Admin' ? (
                  ...
                  <>
                    <div>
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold ${statusColors[showResultModal.status]}`}
                      >
                    ...
                    {!showResultModal.results &&
                      showResultModal.status !== 'Completed' && (
