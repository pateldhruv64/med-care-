    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          {
            label: 'Confirmed',
            value: monthAppts.filter((a) => a.status === 'Confirmed').length,
            color: 'text-green-600 bg-green-50',
          },
          {
            label: 'Pending',
            value: monthAppts.filter((a) => a.status === 'Pending').length,
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base sm:text-lg font-bold text-slate-800 min-w-[140px] sm:min-w-[200px] text-center">
              {monthName}
            </h2>
            <button
              onClick={nextMonth}
          ...
          <button
            onClick={goToday}
            className="px-4 py-1.5 text-sm font-medium text-cyan-600 border border-cyan-200 rounded-lg hover:bg-cyan-50 transition-colors"
          >
            Today
          </button>
        </div>
        ...
        <div className="grid grid-cols-7">
          {calendarDays.map((day, i) => {
            ...
            return (
              <div
                key={i}
                className={`min-h-[60px] sm:min-h-[80px] md:min-h-[100px] border-b border-r border-slate-50 p-1 sm:p-1.5 cursor-pointer transition-colors ${
                  ...
                  <>
                    ...
                    <div className="mt-1 space-y-0.5">
                      {dayAppts.slice(0, 3).map((a) => (
                        <div
                          key={a._id}
                          className={`text-[10px] px-1.5 py-0.5 rounded truncate text-white font-medium ${statusColors[a.status] || 'bg-slate-400'}`}
                        >
            ...
            {selectedAppts.length === 0 ? (
              ...
              selectedAppts
                ...
                .map((a) => (
                  <div
                    key={a._id}
                    className="px-3 sm:px-6 py-3 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 hover:bg-slate-50 transition-colors"
                  >
                    <div
                      className={`w-1 h-10 rounded-full ${statusColors[a.status]}`}
                    ></div>
                    ...
                    <span
                      className={`px-2.5 py-1 rounded-full text-xs font-medium ${statusBadge[a.status]}`}
                    >
                      {a.status}
                    </span>
                  </div>
