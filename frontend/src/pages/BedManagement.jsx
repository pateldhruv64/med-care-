...
const BedManagement = () => {
  ...
  const [dischargingBedId, setDischargingBedId] = useState('');
  ...
  const handleDischarge = async (bed) => {
    const shouldProceed = window.confirm(
      `Discharge patient from Room ${bed.roomNumber}, Bed ${bed.bedNumber}? A billing invoice will be generated.`,
    );
  ...
  const filtered = beds.filter((b) => {
    ...
    if (search) {
      ...
      return (
        ...
        b.patient?.firstName?.toLowerCase().includes(s) ||
        b.patient?.lastName?.toLowerCase().includes(s)
      );
    }
    return true;
  });
    ...
    >
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <select
          ...
          className="border border-slate-200 rounded-lg py-2 px-3 text-sm focus:outline-none focus:border-cyan-400"
        >
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 px-6 py-12 text-center text-slate-500">
          ...
          <p>
            No beds found.{' '}
            {user?.role === 'Admin' && 'Click "Add Bed" to get started.'}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((bed) => (
            <div
              key={bed._id}
              className={`rounded-xl border shadow-sm p-4 ${wardColors[bed.ward] || 'bg-white border-slate-200'}`}
            >
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="flex items-center gap-1.5">
                  <span
                    className={`w-2 h-2 rounded-full ${statusDot[bed.status]}`}
                  ></span>
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs font-medium border ${statusColors[bed.status]}`}
                  >
                    {bed.status}
                  </span>
                </div>
              ...
              <div className="flex gap-2">
                {bed.status === 'Available' && (
                  <button
                    ...
                    className="flex-1 py-1.5 text-xs font-bold bg-cyan-500 text-white rounded-lg hover:bg-cyan-600 flex items-center justify-center gap-1"
                  >
                ...
                {bed.status === 'Occupied' && (
                  <button
                    ...
                    className="flex-1 py-1.5 text-xs font-bold bg-orange-500 text-white rounded-lg hover:bg-orange-600 flex items-center justify-center gap-1 disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                    <LogOut size={12} />{' '}
                    {dischargingBedId === bed._id
                      ? 'Discharging...'
                      : 'Discharge'}
                  </button>
                )}
                {user?.role === 'Admin' && bed.status !== 'Occupied' && (
                  <>
                    <button
                      ...
                      className="py-1.5 px-2 text-xs font-bold border border-slate-200 rounded-lg hover:bg-slate-100 flex items-center gap-1"
                    >
                      <Wrench size={12} />{' '}
                      {bed.status === 'Maintenance' ? 'Fix' : 'Maint.'}
                    </button>
                    <button
                      onClick={() => handleDelete(bed._id)}
                      className="py-1.5 px-2 text-xs font-bold text-red-500 border border-red-200 rounded-lg hover:bg-red-50"
                    >
                      <X size={12} />
                    </button>
                  </>
            ...
            >
              <div className="bg-slate-50 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                  <BedDouble className="text-cyan-500" size={20} /> Add New Bed
                </h2>
                <button
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewBed({ ...newBed, roomNumber: e.target.value })
                      }
                      placeholder="e.g., 101"
            ...
            >
              <div className="bg-slate-50 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
                <h2 className="text-lg font-bold text-slate-800">
                  Assign Patient — Room {showAssignModal.roomNumber}, Bed{' '}
                  {showAssignModal.bedNumber}
                </h2>
                <button
