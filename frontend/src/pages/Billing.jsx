...
const Billing = () => {
  ...
  const markAsPaid = async (invoice) => {
    const invoiceNumber = invoice?._id?.slice(-6) || 'N/A';
    const shouldProceed = window.confirm(
      `Mark invoice #${invoiceNumber} as paid? This action affects billing records.`,
    );
                ...
                <div className="space-y-2 mb-6">
                  ...
                  {invoice.invoiceType && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span
                        className={`font-medium ${invoice.invoiceType === 'Pharmacy' ? 'text-green-600' : 'text-blue-600'}`}
                      >
                        {invoice.invoiceType}
                      </span>
                    </div>
                ...
                {(user.role === 'Admin' || user.role === 'Receptionist') &&
                  invoice.status === 'Unpaid' && (
                    <button
                      ...
                      className="flex-1 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors font-medium text-sm flex items-center justify-center gap-1 disabled:opacity-60 disabled:cursor-not-allowed"
                    >
                      <CheckCircle size={16} />{' '}
