...
const MedicalRecords = () => {
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      try {
        ...
        setAppointments(
          results[1].data.filter((a) => a.status === 'Completed'),
        );
  ...
  const toggleActive = async (record) => {
    ...
    const shouldProceed = window.confirm(
      nextActiveState
        ? `Reactivate "${record.title}" in the patient's active medical history?`
        : `Mark "${record.title}" as resolved?`,
    );
    ...
    try {
      await api.put(`/medical-history/${record._id}`, {
        isActive: nextActiveState,
      });
      toast.success(
        record.isActive ? 'Marked as resolved' : 'Marked as active',
      );
      const { data } = await api.get('/medical-history');
      setMedicalHistory(data);
    } catch (error) {
      toast.error(
        error.response?.data?.message || 'Failed to update record status',
      );
    }
  };
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            label: 'Conditions',
            value: medicalHistory.filter((r) => r.isActive).length,
            color: 'text-red-500',
            bg: 'bg-red-50',
            icon: Heart,
          },
          {
            label: 'Allergies',
            value: medicalHistory.filter((r) => r.type === 'Allergy').length,
          ...
          {medicalHistory.length === 0 ? (
            ...
            medicalHistory.map((record) => {
              ...
              return (
                <div
                  key={record._id}
                  className={`bg-white rounded-xl shadow-sm border overflow-hidden ${!record.isActive ? 'opacity-60' : ''}`}
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          {record.severity !== 'N/A' && (
                            <span
                              className={`px-2 py-0.5 rounded text-xs font-medium ${severityColors[record.severity]}`}
                            >
                              {record.severity}
                            </span>
                          )}
                          <span className="text-xs text-slate-400">
                            ...
                            • Dr. {record.addedBy?.firstName}{' '}
                            {record.addedBy?.lastName}
                          </span>
                        </div>
                      </div>
                    </div>
